# Statistics for Oncology Handbook

This repository is the handbook that accompanies the course ***Statistics for Oncology - A Course for Scottish Trainees*** designed for:

-   **Clinical Oncology trainees** preparing for the FRCR Part 1 examination
-   **Medical Oncology trainees** preparing for the Specialty Certificate Examination (SCE)

The course assumes **no prior knowledge** of statistics

🔗 **Main course page**: [Oncology Statistics – Edinburgh Cancer Informatics](https://cancer-data.ecrc.ed.ac.uk/oncology-statistics/)

## Learning Objectives

-   Understand core statistical concepts relevant to oncology
-   Critically evaluate published oncology-relevant research
-   Answer FRCR Part 1 and SCE examination questions on medical statistics

## Contributing

Contributions, suggestions, and feedback are welcome! Please:

-   Visit the Github repository: <https://github.com/Edinburgh-Cancer-Informatics/onco-stats> and either
    -   Open an issue for suggestions for improvement or
    -   Make the improvements yourself and submit them as a pull request
-   If you can't be bothered with github just contact us via the [main course page](edin.ac/oncology-statistics)

## License

This project is licensed under the MIT License - see the [LICENSE](LICENSE) file for details.

This means you are free to use, modify, and distribute these materials for educational purposes, with attribution.

## Acknowledgments

These materials were originally produced by Phillip McLoon in 2018 and subsequently developed by the Edinburgh Cancer Informatics Group to support oncology trainees across Scotland in developing essential statistical skills for clinical research and professional examinations.

------------------------------------------------------------------------
